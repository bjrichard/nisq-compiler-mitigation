...
def run_experiment(
    ...
    - return_value : tuple[float, float]
        Pair containing noisy error and mitigated error.
    """
    ...
    noisy_error = abs(
        noisy_probs.get("0", 0.0) - ideal_probs["0"]
    )

    mitigated_error = abs(
        mitigated_probs.get("0", 0.0) - ideal_probs["0"]
    )
...
def average_experiment(
    ...
    for _ in range(repeats):
        noisy_error, mitigated_error = run_experiment(
            flip_probability,
            shots,
        )
...
def main() -> None:
    """
    Run a noise sweep and save results.
    ...
    ---------
    - return_value : None
        Prints and saves noise sweep results.
    """
    ...
    for flip_probability in NOISE_SWEEP_LEVELS:
